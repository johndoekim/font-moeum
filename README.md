...
- **코딩 폰트 모드** — [kuskhan/jetendard](https://github.com/kuskhan/jetendard) 방식으로 한글을 라틴 고정폭 셀(기본 2:1)에 맞춰 정렬. 한글 스케일·폭 배수·세로 오프셋 등을 고급 패널에서 조정
...
**코딩 폰트 모드**는 `scripts/fitmerge.py` 전용 엔진으로, 고정폭 A를 베이스로 열고 B의 한글 글리프를 라틴 폭의 정수배 셀(기본 2배)에 맞춰 스케일·중앙정렬해 복사합니다([kuskhan/jetendard](https://github.com/kuskhan/jetendard) 방식). 터미널·에디터처럼 라틴과 한글이 정확히 2:1로 정렬돼야 하는 코딩 폰트에 적합하며, A가 고정폭이 아니면 병합이 거부됩니다. 고급 패널에서 한글 스케일·폭 배수·세로 오프셋·한자 포함 여부·전각 담당·자모 조합(ccmp, 조합형 자모를 완성형 음절로 합성)을 조정할 수 있습니다.
...
- [kuskhan/jetendard](https://github.com/kuskhan/jetendard) — 코딩 폰트 모드의 영감
