...
def is_cff_flavored(path: Path) -> bool:
    """sfnt 태그 4바이트가 'OTTO'(CFF 계열)인지 — 테이블 파싱 없이 배지용 판정."""
    try:
...
def _load_otf_converted(path: Path, font: TTFont) -> TTFont:
    ...
    캐시 유효 = 존재 and 원본보다 엄격히 최신(mtime) and glyf 포함 TTFont로 열림.
    mtime 검사는 필수: 앱의 upload_N 파일명 seq가 세션마다 리셋되는데 work_dir은
    %TEMP%에 지속되어, 같은 경로가 다른 내용으로 재사용될 수 있다.
    변환은 업로드 직후 inspect가 이 경로를 타면서 1회 선지불되므로
    재병합 루프는 캐시를 여는 기존 TTF 속도 그대로다.
    """
    ...
    try:
        ...
        font.save(str(tmp))
        os.replace(tmp, cache)  # 원자적 교체 — 반쯤 쓰인 캐시가 관측되지 않게
    except OSError as e:
