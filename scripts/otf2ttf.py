...
# maxp 0.5(CFF)에는 없는 v1.0 필드. maxZones는 스펙상 1|2만 유효(인스트럭션 없으면 1,
# 0은 검증기가 플래그) — 아래에서 1로 고정. 나머지는 TT 인스트럭션이 없으므로 0이 정답.
# (maxPoints·컴포넌트 계열은 저장 시 recalc가 재계산하지만, recalcBBoxes=False 저장
#  경로에서도 컴파일이 죽지 않도록 방어적으로 0을 깔아둔다)
_MAXP_V1_FIELDS = (
...
def otf_to_ttf(font: TTFont, *, max_err: float | None = None,
    ...
    T2 seac(악센트 합성)는 addComponent로 통과되어 컴포지트 글리프로 보존된다
    (base/accent 참조 글리프도 함께 변환되므로 유효 — 저장 시 recalc가
    maxComponent* 값을 재계산한다).
    """
    if max_err is None:
    ...
    try:
        ...
        # 비표준 이름 65,279개 초과(풀버전 Pan-CJK CID OTF 등)는 2.0 인코딩(uint16)
        # 한계를 넘는다 — 업스트림 스니펫과 동일하게 이름을 포기하고 3.0으로 낙하
        post.formatType = 3.0
